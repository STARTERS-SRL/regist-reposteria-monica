...
export default function PosView() {
  ...
  const verificarEstado = async () => {
    ...
    setEstado(cierre ? 'cerrado' : 'vendiendo')
  }
  ...
  const precargarCantidades = async () => {
    ...
    if (cierreAyer?.snapshot_stock) {
      ...
      lista.forEach((p: any) => { initial[p.id] = snap[String(p.id)] ?? 0 })
    } else {
  ...
  const iniciarDia = async () => {
    if (!sucursalId) { setMensaje('Seleccione una sucursal'); return }
    setCargando(true)
    const hoy = new Date().toISOString().split('T')[0]
    ...
    if (errReg) {
      setMensaje('Error al abrir el dia'); setCargando(false); return
    }
    ...
    setEstado('vendiendo'); setCargando(false)
  ...
  const registrarVenta = async () => {
    ...
    if (errorVenta || !venta) { setMensaje('Error al registrar la venta'); setCargando(false); return }
    ...
    if (errorDetalles) { setMensaje('Error al registrar los detalles'); setCargando(false); return }
    ...
    for (const item of carrito) {
      ...
      if (inv) {
        await supabase.from('inventario').update({ cantidad: inv.cantidad - item.cantidad }).eq('id', inv.id)
      }
    }

    setMensaje('Venta exitosa'); setCarrito([]); setCargando(false)
    setTimeout(() => setMensaje(''), 3000)
  }
  ...
  const confirmarCierre = async () => {
    ...
    if (errReg) { setMensaje('Error al cerrar el dia'); setCargando(false); return }
    ...
    setMensaje('Dia cerrado correctamente'); setCargando(false)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {usuario && <p className="text-sm text-gray-600">{usuario.nombre}</p>}
      </div>
      ...
      {mensaje && (
        <div className={`rounded border p-4 ${mensaje.includes('Error') ? 'bg-red-50 border-red-200 text-red-700' : 'bg-green-50 border-green-200 text-green-700'
          }`}>
      ...
      {estado === 'apertura_pendiente' && sucursalId && (
        <div className="rounded border border-gray-200 bg-white p-6">
          <h3 className="mb-4 text-lg font-semibold text-gray-900">Apertura del Dia</h3>
          <p className="mb-4 text-sm text-gray-600">Ingrese las cantidades iniciales:</p>
          ...
          <button onClick={iniciarDia} disabled={cargando}
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div className="rounded border border-gray-200 bg-white p-4">
              <h3 className="mb-4 text-lg font-semibold text-gray-900">Carrito</h3>
              {carrito.length === 0 ? (
              ...
              <div className="mt-4 space-y-2">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setMetodoPago('efectivo')}
                    className={`flex-1 rounded border px-3 py-2 text-sm font-medium ${metodoPago === 'efectivo'
                        ? 'border-blue-500 bg-blue-50 text-blue-700'
                        : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50'
                      }`}>Efectivo</button>
                  <button onClick={() => setMetodoPago('qr')}
                    className={`flex-1 rounded border px-3 py-2 text-sm font-medium ${metodoPago === 'qr'
                        ? 'border-blue-500 bg-blue-50 text-blue-700'
                        : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50'
                      }`}>QR</button>
                </div>
              ...
              <button onClick={registrarVenta}
                disabled={carrito.length === 0 || cargando || !sucursalId}
      ...
      {estado === 'cerrado' && sucursalId && (
        <div className="rounded border border-gray-200 bg-white p-6">
          <h3 className="mb-4 text-lg font-semibold text-gray-900">Cierre del Dia</h3>
          <p className="mb-4 text-sm text-gray-600">Confirme las cantidades finales:</p>
          <div className="space-y-3 max-w-md">
            {productos.map((p) => (
              <div key={p.id} className="flex items-center gap-4">
                <label className="w-40 text-sm font-medium text-gray-700">{p.nombre}</label>
